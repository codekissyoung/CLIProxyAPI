...
// codexOutputItemsRetainLimit bounds the best-effort response.output repair buffer.
...
func logCodexUpstreamError(ctx context.Context, ident interface{ Identifier() string }, transport, model string, auth *cliproxyauth.Auth, opts cliproxyexecutor.Options, upstreamBody []byte, err error) {
	...
	helps.LogUpstreamErrorDetail(helps.LogWithRequestID(ctx), helps.UpstreamErrorDetail{
		Provider: ident.Identifier(), AuthID: authID, AuthLabel: authLabel,
		Model: model, Status: codexErrorStatus(err), ErrorMsg: err.Error(),
		SessionID: sessionID, Transport: transport, ReqBytes: len(upstreamBody),
	})
}
...
func codexContextRejectBlocked(ctx context.Context, ident interface{ Identifier() string }, transport, model string, auth *cliproxyauth.Auth, opts cliproxyexecutor.Options, body []byte) (error, bool) {
	fingerprint := helps.CodexContextFingerprint(body)
	if fingerprint == "" || !codexContextRejectCache.Blocked(fingerprint, time.Now()) {
		return nil, false
	}
	message := "Your input exceeds the context window of this model. This exact request was already rejected for being too large; please shorten the conversation and try again."
	errBody := fmt.Sprintf(`{"error":{"message":%q,"type":"invalid_request_error","code":"context_too_large"}}`, message)
	authID, authLabel := codexErrorAuthInfo(auth)
	helps.LogUpstreamErrorDetail(helps.LogWithRequestID(ctx), helps.UpstreamErrorDetail{
		Provider: ident.Identifier(), AuthID: authID, AuthLabel: authLabel,
		Model: model, Status: http.StatusBadRequest,
		...
		Transport: transport + "-retry-block", ReqBytes: len(body),
	})
...
func codexContextRejectRecord(body, errorResponseBody []byte) {
	...
	if fingerprint := helps.CodexContextFingerprint(body); fingerprint != "" {
		codexContextRejectCache.Record(fingerprint, time.Now())
	}
}
