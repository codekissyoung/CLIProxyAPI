...
func TestNewUtlsRoundTripperRoutesByHost(t *testing.T) {
	...
	// Non-protected host must go through the fallback, never the fingerprint legs.
	...
	if !IsAnthropicUpstreamURL(anthropicURL) {
		t.Fatal("api.anthropic.com must be recognized as an Anthropic upstream")
	}
}
