...
func (e *CodexExecutor) ExecuteStream(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (_ *cliproxyexecutor.StreamResult, err error) {
	...
	if httpResp.StatusCode < 200 || httpResp.StatusCode >= 300 {
		...
		helps.LogUpstreamErrorDetail(entry, helps.UpstreamErrorDetail{
			Provider: e.Identifier(), Model: req.Model, AuthID: authID, AuthLabel: authLabel,
			Status: httpResp.StatusCode, ErrorMsg: errSummary,
	...
	if buffering {
		for scanner.Scan() {
			...
			if transformed, ok := grokbuild.TransformKeepaliveSSELine(translatedLine, isGrokClient); ok {
				...
				if streamErr, terminalBody, ok := codexTerminalFailureErr(applyCodexIdentityExposeResponsePayload(data, identityState)); ok {
					closeBootstrapBody()
