...
func (e *CodexExecutor) cacheHelper(ctx context.Context, from sdktranslator.Format, url string, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, userPayload []byte, rawJSON []byte, headerSets ...http.Header) (*http.Request, []byte, codexIdentityConfuseState, error) {
	...
	if sourceFormatEqual(from, sdktranslator.FormatClaude) {
		// Resolve the wire model name (falling back to the thinking-suffix-stripped
		// req.Model) and pass the real downstream headers: session/agent IDs resolve
		// from headers first, then fall back to the ctx->gin request, so the prompt
		// cache key stays stable per Claude Code session and is isolated per agent.
