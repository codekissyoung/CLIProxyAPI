...
func TestHostAffinityLookupCallback_Contract(t *testing.T) {
	...
	t.Run("rebinding A to B reflects updated binding", func(t *testing.T) {
		...
		// ice divergence: the Home/temp affinity model keeps the home binding on
		// authA when the second pick can only reach authB as a temporary
		// failover, so the lookup still reports authA.
		if resp.Status != pluginapi.HostAffinityStatusBound || resp.AuthIndex != authA.Index {
			t.Fatalf("lookup after rebinding: got index=%q status=%q, want index=%q (home preserved)", resp.AuthIndex, resp.Status, authA.Index)
		}
	})
	...
	t.Run("extended namespaces resolve bare ID (task, clientreq, geminicache, execution)", func(t *testing.T) {
		...
		for _, tc := range testCases {
			t.Run(tc.name, func(t *testing.T) {
				...
				if errPick != nil {
					t.Fatalf("pick failed: err=%v", errPick)
				}
				// ice divergence: the Home/temp affinity model sequences fresh-home
				// picks differently from upstream's round-robin, so assert the
				// lookup against the credential the pick actually committed.
				wantIndex := authA.Index
				if picked.ID == authB.ID {
					wantIndex = authB.Index
				}
				...
				if respBare.Status != pluginapi.HostAffinityStatusBound || respBare.AuthIndex != wantIndex {
					t.Fatalf("bare lookup %q: got status=%q index=%q, want bound index=%q", tc.bareID, respBare.Status, respBare.AuthIndex, wantIndex)
				}
				...
				if respQual.Status != pluginapi.HostAffinityStatusBound || respQual.AuthIndex != wantIndex {
					t.Fatalf("qual lookup %q: got status=%q index=%q, want bound index=%q", tc.qualID, respQual.Status, respQual.AuthIndex, wantIndex)
				}
			})
		}
	...
	t.Run("disabled credential returns bound with disabled flag", func(t *testing.T) {
		...
		rawResp, _ := host.callFromPlugin(context.Background(), pluginabi.MethodHostAffinityLookup, reqPayload)
		resp, _ := decodeRPCEnvelope[pluginapi.HostAffinityLookupResponse](rawResp)
		// ice divergence: the enabled->disabled transition purges the
		// credential's session-affinity bindings so sessions re-home (pinned by
		// conductor_update_test.go), so the lookup reports unbound instead of
		// upstream's bound+disabled.
		if resp.Status != pluginapi.HostAffinityStatusUnbound {
			t.Fatalf("status = %q, want unbound (disable purges affinity bindings)", resp.Status)
		}
	})
	...
	t.Run("concurrent reads", func(t *testing.T) {
		// ice divergence: the disabled-credential purge above dropped the
		// bound-session-1 binding; re-establish it before the concurrent reads.
		rebindOpts := cliproxyexecutor.Options{
			Headers:  map[string][]string{"X-Claude-Code-Session-Id": {"bound-session-1"}},
			Metadata: make(map[string]any),
		}
		if _, errRebind := selector.Pick(context.Background(), "anthropic", "claude-3-7-sonnet", rebindOpts, []*coreauth.Auth{authA}); errRebind != nil {
			t.Fatalf("rebind pick failed: %v", errRebind)
		}
		reqPayload, _ := json.Marshal(pluginapi.HostAffinityLookupRequest{
			Provider:  "anthropic",
			Model:     "claude-3-7-sonnet",
