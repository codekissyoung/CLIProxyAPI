...
- Production deployment is driven from the `claude-relay-server` control repo: `scripts/cliproxy-update.sh` runs on the control host (ice-db-server), builds this repo's **`ice-pool-guard`** checkout at `~/CLIProxyAPI` once (the branch must be checked out, pushed to origin, and clean), and deploys in rolling order `ice-server-4` → `ice-do-web-1` — the only two active pools — with versioned installs, symlink flip, restart, health/process verification, a per-host relay reload, and release-ledger entries. It is dry-run by default; pass `--execute` (optionally `--host <name>`). Account files (`~/deploy/auths/`) and configs are never touched. The zero-account runtimes on `ice-do-db` / `ice-do-web-2` (decommissioned 2026-08-30), the withdrawn `.75` pool, and the retired hosts `ice-server` / `ice-server-2` / `ice-server-3` must not be deployed.
