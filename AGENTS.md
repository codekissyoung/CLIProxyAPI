...
- **Codex 0.147.0 wire baseline**: `chatgpt.com` HTTPS uses the captured Codex CLI 0.147.0 reqwest/OpenSSL ClientHello over HTTP/1.1 (no ALPN), while Responses WebSocket uses the captured rustls/AWS-LC ClientHello with randomized extension order (also no ALPN). Both paths remain IPv4-only because the VPS IPv6 exit is Cloudflare-risky. Keep UA/Originator/Version in lockstep with `codex-tui/0.147.0`; refresh the offline ClientHello fixtures and `docs/multi-user-pro-account-hardening.md` before changing this baseline.
