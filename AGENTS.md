...
- **Codex 0.153.4 wire baseline**: `chatgpt.com` HTTPS uses the captured Codex CLI 0.147.0 reqwest/OpenSSL ClientHello over HTTP/1.1 (no ALPN), while Responses WebSocket uses the captured rustls/AWS-LC ClientHello with randomized extension order (also no ALPN). Both paths remain IPv4-only because the VPS IPv6 exit is Cloudflare-risky. Keep UA/Originator/Version in lockstep with `codex-tui/0.153.4` (bumped from 0.147.0 on 2026-09-05 because the OpenAI backend hard-400s `gpt-6-astra` for any presented version below ~0.153; the 0.147.0 ClientHello fixtures were deliberately kept — the gate is application-layer and the TLS stack did not change across these versions). Refresh the offline ClientHello fixtures and `docs/multi-user-pro-account-hardening.md` before changing the TLS baseline.
