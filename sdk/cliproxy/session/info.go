...
func ExtractSessionInfo(headers http.Header, payload []byte, metadata map[string]any) (SessionInfo, bool) {
	...
	// ice divergence: native Codex clients mirror the turn-metadata header into
	// the request body field client_metadata.x-codex-turn-metadata when the
	// header itself is absent; upstream only reads the header copy. Extract it
	// ahead of the slot/conv/thread header fallbacks.
	if sid := codexTurnMetadataBodySessionID(payload); sid != "" {
		info.ClientType = "codex"
...
// codexTurnMetadataBodySessionID extracts the codex session_id mirrored in the
// request body field client_metadata.x-codex-turn-metadata (ice divergence).
func codexTurnMetadataBodySessionID(payload []byte) string {
	if len(payload) == 0 {
		return ""
	}
	...
	return normalizedSessionCandidate(gjson.Get(raw, "session_id").String())
}
