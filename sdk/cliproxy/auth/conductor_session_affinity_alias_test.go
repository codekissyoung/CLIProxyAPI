		...
		for _, mode := range []string{"no-session", "explicit-session", "lcp"} {
			for _, path := range []string{"select", "execute", "stream"} {
				t.Run(strategy+"/"+mode+"/"+path, func(t *testing.T) {
					...
					// ice divergence: the Home/temp affinity model returns an
					// explicit session to its recovered home credential instead of
					// sticking to upstream's temporary failover pick.
					if mode == "no-session" || mode == "explicit-session" {
