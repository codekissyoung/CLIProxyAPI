...
func (s *authScheduler) pickSingleWithStrategy(ctx context.Context, provider, model string, opts cliproxyexecutor.Options, tried map[string]struct{}, strategy schedulerStrategy) (*Auth, error) {
	...
	if strategy == schedulerStrategyCurrent {
		strategy = s.strategyFor(providerKey)
	}
...
func (s *authScheduler) pickMixedWithStrategy(ctx context.Context, providers []string, model string, opts cliproxyexecutor.Options, tried map[string]struct{}, strategy schedulerStrategy) (*Auth, string, error) {
	...
	s.mu.Lock()
	defer s.mu.Unlock()
	explicitStrategy := strategy != schedulerStrategyCurrent
	if !explicitStrategy {
		strategy = s.strategy
	}
	providerStrategy := func(providerKey string) schedulerStrategy {
		if explicitStrategy {
			return strategy
		}
		return s.strategyFor(providerKey)
	}
	if pinnedAuthID != "" {
		...
		shard := providerState.ensureModelLocked(modelKey, time.Now())
		predicate := scheduledAuthPredicate(eligibility, tried, pinnedAuthID, providerStrategy(providerKey) == schedulerStrategyWeightedRoundRobin)
		if picked := shard.pickReadyLocked(false, providerStrategy(providerKey), predicate); picked != nil {
			return picked, providerKey, nil
		}
	...
	if strategy == schedulerStrategyFillFirst {
		for providerIndex, providerKey := range normalized {
			...
			picked := shard.pickReadyAtPriorityLocked(false, bestPriority, providerStrategy(providerKey), predicate)
			if picked != nil {
				return picked, providerKey, nil
			}
		}
