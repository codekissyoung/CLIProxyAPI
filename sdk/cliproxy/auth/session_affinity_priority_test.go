...
func TestManagerSessionAffinityPreservesBindingAcrossHigherPriorityRecovery(t *testing.T) {
	for _, testCase := range []struct {
		...
		t.Run(testCase.name, func(t *testing.T) {
			...
			// Make the high-priority credential unavailable before this session gets
			// a home. The low-priority credential therefore becomes the actual home,
			// rather than a temporary fallback from a pre-existing high-priority home.
			...
			if got := pick(opts); got.ID != lowID {
				t.Fatalf("cold binding while high priority unavailable = %q, want %q", got.ID, lowID)
			}
