...
// An established home binding outranks credential priority, while priority still
// decides cold bindings, requests without a session, and temporary failover picks.
func (s *SessionAffinitySelector) Pick(ctx context.Context, provider, model string, opts cliproxyexecutor.Options, auths []*Auth) (auth *Auth, err error) {
	defer func() {
		if err == nil && auth != nil {
			metrics.RecordAccountPick(auth.ID)
		}
	}()
	...
	// Validate the retained home across every priority tier, but expose only the
	// highest currently available tier to cold and temporary fallback selection.
	...
	// Plain conversation fallbacks share one alias group with the primary session.
	// Forks and subagents bind their own keys only, so their failover or release
	// never mutates the parent binding.
	linkAliases := fallbackID != "" && fallbackID != primaryID && !isSubagent && !isFork
	...
	modelKey := homeKey + "::" + canonicalModelKey(model)
	modelAliasKey := ""
	if homeAliasKey != "" {
		modelAliasKey = homeAliasKey + "::" + canonicalModelKey(model)
	}
	...
	if !homeFound && homeAliasKey != "" && (!isSubagent || s.subagentAffinity) {
...
func (s *SessionAffinitySelector) OnResult(res Result) {
	...
	// The home alias of a subagent or fork session is the parent's home key;
	// only release it for plain linked conversations where it is the session's own alias.
	if homeAliasKey != "" && !isSubagentSession(primaryID, fallbackID) {
