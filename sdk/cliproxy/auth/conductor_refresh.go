...
func (m *Manager) refreshAuthForRequest(ctx context.Context, id, failedAccessToken string) (*Auth, error) {
	...
	if err != nil {
		...
		if current := m.auths[id]; current != nil {
			...
			// not marked unavailable, so the previous LastError is the transition
			// marker there.
			previousUnauthorized := current.LastError != nil &&
				(current.LastError.StatusCode() == http.StatusUnauthorized || strings.EqualFold(current.LastError.Code, "unauthorized"))
			recordRevocation := unauthorized && !previousUnauthorized &&
				(!current.Unavailable || current.Status != StatusError)
